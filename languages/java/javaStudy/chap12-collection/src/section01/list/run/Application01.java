...
public class Application01 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * Collection Framwork는 크게 3가지 인터페이스 중 한 가지를 상속받아 구현해 놓았다.
		 * 1. list 인터페이스
		 * 2. Set 인터페이스
		 * 3. Map 인터페이스
		 * 
		 ...
		 *    - Vector, ArrayList, LinkdList, stack Queue 등이 있다.
		 * 2. Set 인터페이스 
		 ...
		 * add() clear() size() contains() equals() isEmpty()
		 ...
		 * 배열의 크기를 변경할 수 없고, 요소의 추가, 삭제, 정렬 등이 복잡하다는 단점을 가지고 있따.
		 ...
		 * 내부적으로 관ㄹ되는 배열의 사이즈는 외부에서 알 필요가 없기 때문에 기능을 제공하지 않는다.
		 * */
		 ...
		 * 배열과 같이 인덱스로 요소들을 관리하기 때문에 인덱스가 다른 위치에 동일한 값을 저장하는 것이 가능ㅎ다ㅏ.
		 * */
		System.out.println();
		alist.add("apple");
		System.out.println("alist: " + alist);
		
		/* 원하는 인덱스의 위치에 값을 추가할 수도 있다.
		 * 값을 중간에 추가하는 경우 인덱스의 위치에 덮어쓰는 것이 아닌
		 * 새로운 값이 들어가는 인덱스의 위치에 값을 넣고 이후 인덱스는 하나씩 뒤로 밀리게 된다.
		 * */
		System.out.println();
		alist.add(1, "banana");
		System.out.println("alist : "+ alist);
...
//		stringList.add(123);
		...
		/* 조급 복잡하지만 내림차순 정렬을 할 수 도 있다.
		 * 하지만 기본적으로 ArrayList에는 역순으로 정렬하는 기능이 제공되지 않는다.
		 * 역순 정렬 기능은 linkedList에 정의되어 있는데, 현재 사용하는 ArrayList를 LinkedList로 변경할 수 있다.
		 * */
		 ...
		 * 컬렉션에서 값을 읽어오는 방식을 통일된 방식으로 제공하기 위해 사요한다.
		 * 반복자 라고 불리우며, 반복문을 이용해서 목록을 하나씩 꺼내는 방식으로 사용하기 위함이다.
		 * 인덱스로 관리되는 컬렉션이 아닌 경우에는 반목문을 사용해서 요소에 하나씩 접근할 수 없기 때문에
