...
public class Application01 {
	...
	public static void main(String[] args) {
		 ...
		 * StringBuilder : 가변이라는 특성을 가지고 있따.
		 *                 문자열에 append() 메소드를 이용하여 합치기 하는 경우 
		 *                 기존 인스턴스를 수정하기 때문에 새로운 인스턴스를 생성하지 않는다.
		 *                 따라서 잦은 문자열 변경이 일어아는 경우 String보다 성능이 좋다.
		 *                 
		 * 단 jdk 1.5 버전에서는 문자열의 + 연산이 stringBuilder의 append()로 컴파일 된다.
		 * 따라서 성능에 큰 차이를 보이지 않는다.
		 * 하지만 반복문에서 문자열의 + 연산을 수행하는 경우 StringBuilder인스턴스를 
