...
public class KindsOfVariable {
	 ...
	 * 필드 == 맴버변수(클래스가 가지는 멤버라는 의미) == 전역변수(클래스 전역에서 사용할 수 있는 변수라는 의미)
	 * 
	...
	public void testMethod(int num) {
		 ...
		 * 메소드의 괄호 안에 선어하는 변수를 매개변수라고 한다.
		 * 매개변수도 일종의 지역변수로 생각하면 된다.
		 * 지역변수와 매개변수 모두 메소드 호출 시 stack을 할당받아 stack에 생성된다.*/
		
		int localNum;
		
		System.out.println(num); // 매개변수 호출 시 값이 넘어와서 변경되기 때문에 초기화가 필요 없다.
//		System.out.println(localNum);
		
		System.out.println(globalNum); // 전역 변수는 클래스 전역에서 사용 가능한다.
		System.out.println(staticNum);
	}
	
	public void testMethod2() {
//		System.out.println(localNum); // 지역변수의 해당 지역(블록, 스코프) 내에서만 사용 가능하다.
