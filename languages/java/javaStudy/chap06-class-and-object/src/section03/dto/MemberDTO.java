...
public class MemberDTO {
	...
	/* 취급하려고 하는 회원정보를 구상해서 필드를 우성 작성해본다.
	 * 회원번호, 회원명, 나이, 성별, 키, 몸무게, 회원탈퇴여부(활성화여부)를 관리할 것이다.
	 * 값 객체가 가지는 속성(필드)를 추출하는 과정 또한 추상화라고 볼 수 있다.
	 * VO클래스를 만들기 위해서는 모든 필드를 private로 만든다.
	 * */
	
	/*필드명은 어떠한 속성을 관리하는 필드인지를 명확하게 명명하는게 좋다. 
	...
	/* 이렇게 필드만 만들고 나면 private로 접근이 제한되어 있기 대문에 각 영역에 접근할 수 없다.
