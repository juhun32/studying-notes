...
public class Application01 {
	public static void main(String[] agrs) {
		 ...
		 * 본인의 상태를 스스로 제어하고 행동하도록 한다는 것이 현실 세계와 다른 점이다.(캡슐화의 의미)
		...
		/* 객체와 객체의 상호작용이란?
		 * 객체와 객체는 메세지를 통해 서로 상호작용을 한다. 메세지(메서드 호출)을 보내는 쪽은 송신자라고 하고
		 * 메서지를 수진하는 쪽을 수신자 라고 한다. 수신자는 메세지를 전달 받아 그 메세지에 해당하는 내용을 처리하는 방법을 
		 ...
		 * 1. 자동차는 처음 멈춘 상태로 대기하고 있는다.
		 * 2. 카레이서는 먼저 자동차에 시동을 건다. 이미 걸려있는 경우 다시 시동을 걸 수 없다.
		 * 3. 카레이서가 엑셀레이터를 밟으면 시동이 걸린 상태일 경우 자동차는 시속 10km/h 증가하며 앞으로 나간다.
		 ...
		 * 에러가 나더라도 작성하고 그 에러나는 부분을 따라가며 작성을 해볼 것이다.
		 * */
		
		CarRacer cracer = new CarRacer();
		...
		while(true) {
			...
			switch(no) {
				case 1 : cracer.startUp(); break;
				case 2 : cracer.stepAccelator(); break;
				case 3 : cracer.stepBreak(); break;
				case 4 : cracer.turnOff(); break;
				case 9 : System.out.println("프로그램을 종료 합니다."); break roop;
