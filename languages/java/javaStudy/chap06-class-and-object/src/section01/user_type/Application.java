...
public class Application {
	...
	public static void main(String[] args) {
		 ...
		 * 이제 조금더 복잡한 자료를 취급할 수 있는 방법을 생각해보자
		 * 회원 정보를 관리하기 위해 여러 정보(아이디, 비밀번호, 이름, 나이, 성별, 취미)를 취급하여 관리하려고 한다.
		 * 지금까지 배운 내용들을 가지고 한 번 값을 저장하고 출력해보자*/
		/* 회원의 정보를 관리하고자 하며 값은 아래와 같다.
		 * 1. 아이디
		 * 2. 비밀번호
		 * 3. 이름
		 * 4. 나이
		...
		System.out.println("--------------------------------------");
		/* 이렇게 각가의 변수로 관리하게 되면 여러 가지 단점이 있다.
		 ...
		 * heap에 생성되기 때문에 jvm 기본값이 초기화 된다.
		 * 
		...
		System.out.println("---------------------------");
		Member lsw = new Member();
		lsw.id ="lee";
		lsw.pwd = "123";
		lsw.name = "이상우";
		lsw.gender ='남';
		lsw.hobby = new String[]{"공부", "헬스"};
		
		System.out.println("lsw.id " + lsw.id);
		System.out.println("lsw.pwd " + lsw.pwd);
		System.out.println("lsw.name " + lsw.name);
		System.out.println("lsw.gender " + lsw.gender);
		System.out.println("lsw.hobby " );
		for(String x : lsw.hobby) {
