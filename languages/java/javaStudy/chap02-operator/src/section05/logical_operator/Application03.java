...
public class Application03 {
	...
	public static void main(String[] args) {
		...
		int num1 = 10;
		int num2 = 9;
		/* 앞에 조건이 false로 강제화 해 두고 && 뒤의 조건은 선치 증가로 값을 증가시킨 후 별 의미 없는 조건식으로 만들어 놓았다.
		 * num1 선치 증가 코드가 등장했다면 result1은 11일 것이고, 동작하지 않았다면 결과는 10일 것이다.
		 * */
		int result1 = (false && ++num1 > 0) ? num1 : num2;
		
		System.out.println("&&실행 확인 : " + result1);

		/* 2. 논리식 || 논리식 : 앞의 결과가 true이면 뒤를 실행하지 않음
		...
		/* 앞에 조건을 true로 강제화 해 두고 || 뒤의 조건은 선치 ㅣ증가로 값을 증가시킨 후 별 의미 없는 조건식으로 만들어 놓았다.
		 * num3 선치 증가 코드가 등장했다면 result2의 값은 11이 될 것이고, 동작하지 않았다면 10일 것이다.
		 * */
		int reuslt = (false || num3++ == 10) ? num3 : num4;
		int reuslt2 = (false || ++num3 == 10) ? num3 : num4;
		System.out.println("||실행 확인 : " + reuslt);
		System.out.println("실행 확인 : " + reuslt2);
	}
